    ...
    namespace core {
        ...
        */
        ...
        class string {
            ...
            //! Constructor
            string(const string<T, TAlloc> &other) : array(0), allocated(0),
                used(0)
            {
                *this = other;
            }
            ...
            explicit string(const double number) : array(0), allocated(0),
                used(0)
            {
                c8 tmpbuf[255];
            ...
            explicit string(unsigned int number) : array(0), allocated(0),
                used(0)
            {
                // temporary buffer for 16 numbers
            ...
            explicit string(unsigned long number) : array(0), allocated(0),
                used(0)
            {
                // temporary buffer for 16 numbers
            ...
            string<T, TAlloc> &append(const T *const other,
            ...
            {
                start = core::clamp(start < 0 ? (s32)(used) - 2 : start, 0,
                    (s32)(used) - 2);
            ...
            string<T> subString(u32 begin, s32 length, bool make_lower = false
            ) const
            {
            ...
            string<T, TAlloc> &replace(const string<T, TAlloc> &toReplace,
            ...
            string<T, TAlloc> &trim(
                const string<T, TAlloc> &whitespace = " \t\n\r"
            )
            {
            ...
            */
            ...
            u32 split(container &ret, const T *const c, u32 count = 1,
                bool ignoreEmptyTokens = true, bool keepSeparators = false
            ) const
            {
                if (!c)
                    return 0;
                    ...
                    for (u32 j = 0; j < count; ++j) {
                        if (array[i] == c[j]) {
                            if ((!ignoreEmptyTokens || i - lastpos != 0) &&
                                !lastWasSeparator)
                                ret.push_back(string<T, TAlloc>(&array[lastpos],
                                    i - lastpos));
                            foundSeparator = true;
