    ...
    namespace core {
        ...
        */
        ...
        class vector3d {
            ...
            //! Copy constructor
            vector3d(const vector3d<T> &other) : X(other.X), Y(other.Y),
                Z(other.Z)
            {
            }
            ...
            bool operator<=(const vector3d<T> &other) const
            {
                return (X < other.X || core::equals(X, other.X)) ||
                    ...
                    (core::equals(X, other.X) && core::equals(Y, other.Y) &&
            ...
            Note that this is the opposite direction of interpolation to getInterpolated_quadratic()
            */
            vector3d<T> &interpolate(const vector3d<T> &a, const vector3d<T> &b,
                f64 d
            )
            {
            ...
            */
            vector3d<T> getSphericalCoordinateAngles() const
            {
                ...
                if (length) {
                    ...
                    angle.X = (T)(
                        acos(Y * core::reciprocal_squareroot(length)) *
                            RADTODEG64);
                }
                return angle;
            }
            ...
            vector3d<T> rotationToDirection(
                const vector3d<T> &forwards = vector3d<T>(0, 0, 1)) const
            {
                ...
                const f64 pseudoMatrix[] = {(cp * cy), (cp * sy), (-sp),
                    (srsp * cy - cr * sy), (srsp * sy + cr * cy), (sr * cp),
                    (crsp * cy + sr * sy), (crsp * sy - sr * cy), (cr * cp)};

                return vector3d<T>((T)(forwards.X * pseudoMatrix[0] +
        ...
        inline vector3d<s32> vector3d<s32>::getSphericalCoordinateAngles() const
        {
            ...
            if (length) {
                if (X != 0) {
                    angle.Y = round32(
                        (f32)(atan2((f64)Z, (f64)X) * RADTODEG64));
                } else if (Z < 0)
                    angle.Y = 180;
