...
namespace irr {
    namespace core {
        ...
        class vector2d {
            ...
            vector2d(const dimension2d<T> &other) : X(other.Width),
                Y(other.Height)
            {
            }
            ...
            vector2d<T> &rotateBy(f64 degrees,
            ...
            f64 getAngleTrig() const
            {
                ...
                if (Y > 0)
                    if (X > 0)
                        ...
                        return 180.0 -
                            atan((irr::f64)Y / -(irr::f64)X) * RADTODEG64;
                else if (X > 0)
                    return 360.0 -
                        atan(-(irr::f64)Y / (irr::f64)X) * RADTODEG64;
                else
            ...
            inline f64 getAngle() const
            {
                ...
                const f64 tmp = core::clamp(Y / sqrt((f64)(X * X + Y * Y)),
                    -1.0, 1.0);
                const f64 angle =
                    atan(core::squareroot(1 - tmp * tmp) / tmp) * RADTODEG64;
            ...
            inline f64 getAngleWith(const vector2d<T> &b) const
            {
                ...
                tmp = tmp / core::squareroot(
                    (f64)((X * X + Y * Y) * (b.X * b.X + b.Y * b.Y)));
                if (tmp < 0.0)
                    tmp = -tmp;
            ...
            Note that this is the opposite direction of interpolation to getInterpolated_quadratic()
            */
            vector2d<T> &interpolate(const vector2d<T> &a, const vector2d<T> &b,
                f64 d
            )
            {
        ...
        template<class T>
        dimension2d<T>::dimension2d(const vector2d<T> &other) : Width(other.X),
            Height(other.Y)
        {
        }
