        ...
        */
        ...
        class array {
            ...
            array() : data(0), allocated(0), used(0),
                strategy(ALLOC_STRATEGY_DOUBLE), free_when_destroyed(true),
                is_sorted(true)
            {
            }
            ...
            */
            ...
            void setAllocStrategy(
                eAllocStrategy newStrategy = ALLOC_STRATEGY_DOUBLE
            )
            {
            ...
            void insert(const T &element, u32 index = 0)
            {
                ...
                if (used + 1 > allocated) {
                    ...
                    case ALLOC_STRATEGY_DOUBLE:
                        newAlloc = used + 1 +
                            (allocated < 500 ? (allocated < 5 ? 5 : used) :
                                used >> 2);
                        break;
                    default:
            ...
            const array<T, TAlloc> &operator=(const array<T, TAlloc> &other)
            {
                ...
                if (other.allocated == 0)
                    ...
                    data = allocator.allocate(
                        other.allocated); // new T[other.allocated];
            ...
            void erase(u32 index, s32 count)
            {
                ...
                for (i = index + count; i < used; ++i) {
                    if (i - count >=
                        index + count)    // not already destructed before loop
                        allocator.destruct(&data[i - count]);

                    allocator.construct(&data[i - count],
                        data[i]); // data[i-count] = data[i];
            ...
            void swap(array<T, TAlloc> &other)
            {
                ...
                core::swap(allocator,
                    other.allocator);    // memory is still released by the same allocator used for allocation
                eAllocStrategy helper_strategy(
                    strategy);    // can't use core::swap with bitfields
                strategy = other.strategy;
