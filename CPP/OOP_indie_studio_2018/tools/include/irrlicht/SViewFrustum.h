...
namespace irr {
    namespace scene {
        ...
        */
        inline core::matrix4 &SViewFrustum::getTransform(
            video::E_TRANSFORMATION_STATE state
        )
        {
            u32 index = 0;
        ...
        /*!
            View Frustum depends on Projection & View Matrix
        */
        inline const core::matrix4 &SViewFrustum::getTransform(
            video::E_TRANSFORMATION_STATE state
        ) const
        {
            u32 index = 0;
        ...
        inline bool SViewFrustum::clipLine(core::line3d<f32> &line) const
        {
            ...
            for (u32 i = 0; i < VF_PLANE_COUNT; ++i) {
                if (planes[i].classifyPointRelation(line.start) ==
                    core::ISREL3D_FRONT) {
                    line.start = line.start.getInterpolated(line.end,
                        planes[i].getKnownIntersectionWithLine(line.start,
                            line.end));
                    wasClipped = true;
                }
                if (planes[i].classifyPointRelation(line.end) ==
                    core::ISREL3D_FRONT) {
