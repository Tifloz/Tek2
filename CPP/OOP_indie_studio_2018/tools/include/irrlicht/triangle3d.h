...
namespace irr {
    namespace core {
        ...
        class triangle3d {
            ...
            triangle3d(vector3d<T> v1, vector3d<T> v2, vector3d<T> v3) : pointA(
                v1), pointB(v2), pointC(v3)
            {
            }
            ...
            bool isTotalInsideBox(const aabbox3d<T> &box) const
            {
                return (box.isPointInside(pointA) &&
                    box.isPointInside(pointB) && box.isPointInside(pointC));
            }
            ...
            core::vector3d<T> closestPointOnTriangle(const core::vector3d<T> &p
            ) const
            {
                const core::vector3d<T> rab = line3d<T>(pointA,
                    pointB).getClosestPoint(p);
                const core::vector3d<T> rbc = line3d<T>(pointB,
                    pointC).getClosestPoint(p);
                const core::vector3d<T> rca = line3d<T>(pointC,
                    pointA).getClosestPoint(p);
            ...
            bool isPointInsideFast(const vector3d<T> &p) const
            {
                ...
                return (u > -ROUNDING_ERROR_f32) && (v >= 0) &&
                    (u + v < 1 + ROUNDING_ERROR_f32);
            }
            ...
            ) const
            {
                ...
                const vector3d<f64> linePointf64(linePoint.X, linePoint.Y,
                    linePoint.Z);
                const vector3d<f64> lineVectf64(lineVect.X, lineVect.Y,
                    lineVect.Z);
                vector3d<f64> outIntersectionf64;

                core::triangle3d<irr::f64> trianglef64(vector3d<f64>((f64)
                    pointA.X, (f64)pointA.Y, (f64)pointA.Z)
                ,vector3d<f64>((f64)
                pointB.X, (f64)
                pointB.Y, (f64)
                pointB.Z)
            ...
            T getArea() const
            {
                return (pointB - pointA).crossProduct(
                    pointC - pointA).getLength() * 0.5f;
            }
            ...
            bool isOnSameSide(const vector3d<f64> &p1, const vector3d<f64> &p2,
                const vector3d<f64> &a, const vector3d<f64> &b
            ) const
            {
                ...
                if (res < 0) {
                    ...
                    vector3d<f64> cp1 = bminusa.normalize().crossProduct(
                        (p1 - a).normalize());
