...
namespace irr {
    namespace core {
        ...
        class CMatrix4 {
			...
			\param constructor Choose the initialization style */
            CMatrix4(const CMatrix4<T> &other,
            ...
            //! Make an inverted rotation matrix from Euler angles.
            /** The 4th row and column are unmodified. */
            inline CMatrix4<T> &setInverseRotationDegrees(
                const vector3d<T> &rotation
            );

            //! Make a rotation matrix from angle and axis, assuming left handed rotation.
            /** The 4th row and column are unmodified. */
            inline CMatrix4<T> &setRotationAxisRadians(const T &angle,
            ...
            void rotateVect(core::vector3df &out, const core::vector3df &in
            ) const;
            ...
            CMatrix4<T> &buildProjectionMatrixPerspectiveFovInfinityLH(
            ...
            CMatrix4<T> &buildCameraLookAtMatrixLH(const vector3df &position,
                const vector3df &target, const vector3df &upVector
            );

            //! Builds a right-handed look-at matrix.
            CMatrix4<T> &buildCameraLookAtMatrixRH(const vector3df &position,
                const vector3df &target, const vector3df &upVector
            );
			...
			*/
            ...
            bool equals(const core::CMatrix4<T> &other,
	...
	*/
	...
	be equivalent, i.e. will have the same result when used to rotate a vector or node. */
        ...
        inline core::vector3d<T> CMatrix4<T>::getRotationDegrees() const
        {
            ...
            const core::vector3d<f64> invScale(core::reciprocal(scale.X),
        ...
        template<class T>
        inline CMatrix4<T> &CMatrix4<T>::setRotationAxisRadians(const T &angle,
            const vector3d<T> &axis
        )
        {
	...
	*/
        ...
        inline bool CMatrix4<T>::isIdentity() const
        {
			...
			return true;
            #endif
            if (!core::equals(M[12], (T)0) || !core::equals(M[13], (T)0) ||
                !core::equals(M[14], (T)0) || !core::equals(M[15], (T)1))
                return false;

            if (!core::equals(M[0], (T)1) || !core::equals(M[1], (T)0) ||
                !core::equals(M[2], (T)0) || !core::equals(M[3], (T)0))
                return false;

            if (!core::equals(M[4], (T)0) || !core::equals(M[5], (T)1) ||
                !core::equals(M[6], (T)0) || !core::equals(M[7], (T)0))
                return false;

            if (!core::equals(M[8], (T)0) || !core::equals(M[9], (T)0) ||
                !core::equals(M[10], (T)1) || !core::equals(M[11], (T)0))
                return false;
            /*
...
*/
	...
	*/
        ...
        //! An alternate transform vector method, writing into an array of 3 floats
        template<class T>
        inline void CMatrix4<T>::rotateVect(T *out, const core::vector3df &in
        ) const
        {
        ...
        template<class T>
        _IRR_DEPRECATED_ inline void CMatrix4<T>::transformBox(
	...
	*/
        ...
        )
        {
            ...
            const f32 dx = -0.5f +
                ((viewport.UpperLeftCorner.X + viewport.LowerRightCorner.X) *
                    0.5f);
            const f32 dy = -0.5f +
                ((viewport.UpperLeftCorner.Y + viewport.LowerRightCorner.Y) *
                    0.5f);
