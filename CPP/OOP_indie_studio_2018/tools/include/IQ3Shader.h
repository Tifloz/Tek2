        ...
        namespace quake3 {
            ...
            */
            ...
            struct Q3LevelLoadParameter {
                Q3LevelLoadParameter() : defaultLightMapMaterial(
                    video::EMT_LIGHTMAP_M4),
                    defaultModulate(video::EMFN_MODULATE_4X),
                    defaultFilter(video::EMF_BILINEAR_FILTER),
                    patchTesselation(8), verbose(0), startTime(0), endTime(0),
                    mergeShaderBuffer(1), cleanUnResolvedMeshes(1),
                    loadAllShaders(0), loadSkyShader(0), alpharef(1),
                    swapLump(0),
            ...
            inline s16 isEqual(const core::stringc &string, u32 &pos,
                const c8 *list[], u16 listSize
            )
            {
            ...
            inline core::vector3df getAsVector3df(const core::stringc &string,
                u32 &pos
            )
            {
                core::vector3df v;
            ...
            /*
                extract substrings
            */
            inline void getAsStringList(tStringList &list, s32 max,
                const core::stringc &string, u32 &startPos
            )
            {
                list.clear();
            ...
            struct SBlendFunc {
                SBlendFunc(video::E_MODULATE_FUNC mod) : type(video::EMT_SOLID),
                    modulate(mod), param0(0.f), isTransparent(0)
                {
                }
            ...
            */
            inline static void getBlendFunc(const core::stringc &string,
            ...
            )
            {
                ...
                static const c8 *funclist[] = {"gl_zero", "gl_one",
                    "gl_dst_color", "gl_one_minus_dst_color", "gl_src_color",
                    "gl_one_minus_src_color", "gl_src_alpha",
                    "gl_one_minus_src_alpha", "gl_dst_alpha",
                    "gl_one_minus_dst_alpha", "gl_src_alpha_sat",
                ...
                case video::EBF_ZERO:
                    ...
                    case video::EBF_SRC_COLOR:
                        ...
                        blendfunc.param0 = video::pack_textureBlendFunc(
                            video::EBF_DST_COLOR, video::EBF_ZERO,
                            blendfunc.modulate);
                ...
                if (0 == resolved) {
                    ...
                    blendfunc.param0 = video::pack_textureBlendFunc(
                        ...
                        (video::E_BLEND_FACTOR)dstFact, blendfunc.modulate);
            ...
            struct Noiser {
                static f32 get()
                {
                    ...
                    f32 value = ((f32)(RandomSeed & 0x7FFF) *
                        (1.0f / (f32)(0x7FFF >> 1))) - 1.f;
                    return value;
                }
            };
            ...
            struct SModifierFunction {
                SModifierFunction() : masterfunc0(UNKNOWN),
                    masterfunc1(UNKNOWN), func(SINUS), tcgen(TEXTURE),
                    rgbgen(IDENTITY), alphagen(UNKNOWN), base(0), amp(1),
                    phase(0), frequency(1), wave(1), x(0), y(0), z(0), count(0)
                {
                }
            ...
            inline core::vector3df getMD3Normal(u32 i, u32 j)
            {
                ...
                return core::vector3df(cosf(lat) * sinf(lng),
                    sinf(lat) * sinf(lng), cosf(lng));
            }

            //
            inline void getModifierFunc(SModifierFunction &fill,
                const core::stringc &string, u32 &pos
            )
            {
                if (string.size() == 0)
                    return;

                static const c8 *funclist[] = {"sin", "cos", "square",
                    "triangle", "sawtooth", "inversesawtooth", "noise"};

                fill.func = (eQ3ModifierFunction)isEqual(string, pos, funclist,
                    7);
                fill.func = fill.func == UNKNOWN ? SINUS :
                    (eQ3ModifierFunction)((u32)fill.func +
                        WAVE_MODIFIER_FUNCTION + 1);
                ...
                u32 isDefined(const c8 *name, const c8 *content = 0) const
                {
                    for (u32 i = 0; i != Variable.size(); ++i) {
                        if (0 == strcmp(Variable[i].name.c_str(), name) &&
                            (0 == content ||
                                strstr(Variable[i].content.c_str(), content))) {
                            return i + 1;
                        }
                    }
                    return 0;
            ...
            struct IShader {
                ...
                {
                    if (0 == VarGroup ||
                        stage >= VarGroup->VariableGroup.size())
                        return 0;
            ...
            */
            inline void dumpVarGroup(core::stringc &dest,
                const SVarGroup *group, s32 stack
            )
            {
                core::stringc buf;
                s32 i;
            ...
            */
            inline core::stringc &dumpShader(core::stringc &dest,
                const IShader *shader, bool entity = false
            )
            {
                if (0 == shader)
                    return dest;
            ...
            */
            inline void getTextures(tTexArray &textures,
                ...
                io::IFileSystem *fileSystem, video::IVideoDriver *driver
            )
            {
                static const char *extension[] = {".jpg", ".jpeg", ".png",
                    ".dds", ".tga", ".bmp", ".pcx"};
                    ...
                    for (u32 g = 0; g != 7; ++g) {
                        ...
                        if (loadFile == "$whiteimage") {
                            ...
                            if (0 == texture) {
                                ...
                                u32 image[4] = {0xFFFFFFFF, 0xFFFFFFFF,
                                    0xFFFFFFFF, 0xFFFFFFFF};
                                video::IImage *w = driver->createImageFromData(
                                    video::ECF_A8R8G8B8, s, &image);
                            ...
                            if (0 == texture) {
                                core::dimension2du s(2, 2);
                                u32 image[4] = {0xFFFF0000, 0xFFFF0000,
                            ...
                            if (0 == texture) {
                                core::dimension2du s(2, 2);
                                u32 image[4] = {0xFF0000FF, 0xFF0000FF,
                            ...
                            if (0 == texture) {
                                core::dimension2du s(2, 2);
                                u32 image[4] = {0xFFFFFFFF, 0xFF000000,
                                    0xFF000000, 0xFFFFFFFF};
                                video::IImage *w = driver->createImageFromData(
                                    video::ECF_A8R8G8B8, s, &image);
                                texture = driver->addTexture("$checkerimage",
                                    w);
                                w->drop();
                            }
