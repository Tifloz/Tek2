...
options =
    [ Option ['c'] ["check"] (NoArg (\opts -> Right opts { flag = Check })) "check if a sequence is a de Bruijn sequence,"
    , Option ['u'] ["unique"] (NoArg (\opts -> Right opts { flag = Unique })) "check if 2 sequences are distinct de Bruijn sequences"
